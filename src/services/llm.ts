...
1. create_task — создать задачу (по умолчанию, если из фразы непонятно другое действие):
   { "action": "create_task", "title": "текст задачи", "assignTo": "husband" | "wife" | "me" | "both", "notify": true | false, "periodHours": число }
   assignTo: "me" = кто пишет, "husband" = Лёша, "wife" = Таня, "both" = обоим. notify: true если просят напоминания. Период только от дня: periodHours: 24 = день, 48 = 2 дня, 72 = 3 дня, 168 = неделя. Минимум 24. Если просят "каждую минуту/час" — верни periodHours: 24. Если период не ясен — не указывай periodHours (будет неделя).
...
6. set_reminder_period — напоминать о задаче регулярно (каждые N дней/недель), минимум 1 день:
   { "action": "set_reminder_period", "taskTitle": "заголовок", "periodHours": число }
   periodHours: 24=день, 48=2 дня, 72=3 дня, 168=неделя. Только 24 и больше.

7. set_reminder_once — одно напоминание через N дней или в дату (минимум 1 день):
   { "action": "set_reminder_once", "taskTitle": "заголовок", "inHours": число } — inHours минимум 24
   или { "action": "set_reminder_once", "taskTitle": "заголовок", "atTime": "ISO дата-время" }
   Примеры: "напомни через 2 дня про X" -> inHours: 48, "напомни завтра в 10" -> atTime.
